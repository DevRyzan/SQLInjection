...
class PaymentRepository:
    ...
    def create_payment_info(self, user_id, first_name, last_name, address, credit_card_info_id, type, status):
        query = f"""INSERT INTO payment_info (user_id, first_name, last_name, address, credit_card_info_id, type, status)
                    VALUES ('{user_id}', '{first_name}', '{last_name}', '{address}', '{credit_card_info_id}', '{type}', '{status}')"""
        try:
            self.cursor.execute(query)
            self.connection.commit()
    ...
    def get_payment_info(self, payment_id):
        query = f"SELECT * FROM payment_info WHERE id = {payment_id}"
        try:
            self.cursor.execute(query)
