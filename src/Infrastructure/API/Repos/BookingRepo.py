...
class BookingRepository:
    ...
    def create_booking(self, start_date, end_date, user_id, payment_info_id=None):
        query = """INSERT INTO bookings (start_date, end_date, user_id, payment_info_id)
        VALUES (?, ?, ?, ?)"""
        try:
            self.cursor.execute(query, (start_date, end_date, user_id, payment_info_id))
            self.connection.commit()
    ...
    def get_booking(self, booking_id):
        ...
        try:
            self.cursor.execute(query, (booking_id,)) # Don't remove this comma or it breaks
