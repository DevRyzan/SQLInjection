...
class CreditCardRepository:
    ...
    def create_credit_card(self, user_id, credit_card_number, name_on_card, expr_date, cvv):
        query = """INSERT INTO credit_card_info (user_id, credit_card_number, name_on_card, expr_date, cvv)
           VALUES (?, ?, ?, ?, ?)"""

        try:
            self.cursor.execute(query, (user_id, credit_card_number, name_on_card, expr_date, cvv))
            self.connection.commit()
    ...
    def get_credit_card_info(self, card_id):
        query = "SELECT * FROM credit_card_info WHERE id = ?"
        try:
